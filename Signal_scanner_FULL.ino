...
// Modes: Wi-Fi Scan + BLE Scan + Voltage Meter + Current Meter + RF Sniffer (AD8317)
// ---------------------------------------------------------------------
// Hardware
...
//
// RF Sniffer (AD8317):
//  - Vout (0.2–1.8V typ) -> ADC36 (no divider)
//  - GND common, Vcc per module (~5V), antenna/probe on RF input
//
// Controls
//  - MODE (GPIO2):
//      * Short press: CHANGE MODE (WiFi → BLE → Volt → Curr → RF → …)
...
//
// Splash:
//  - LCD: "Hello use", "Welcome to the Signal Scanner V1.0", "Initializing..."
...
#define PIN_RF_ADC         33    // ADC1_CH0 (RF Vout)
...
enum ModeVals : uint8_t { WIFI_MODE=0, BLE_MODE, VOLT_MODE, CURR_MODE, RF_MODE, MODE_COUNT };
...
static const float EXTRA_DIV       = 1.0f;    // no extra divider
...
// ===================== RF Sniffer (AD8317) =====================
//static const float VREF_ADC        = 3.30f;   // ADC with 11dB attenuation
static const int   ADC_SAMPLES_RF  = 64;
static const float EMA_ALPHA_RF    = 0.18f;

// AD8317 default model until calibrated
...
//static float adcToVoltsRF(int raw) { return (raw/4095.0f)*VREF_ADC; }
...
static void enterMode(uint8_t m) {
  ...
  lcd.clear();
  switch (m) {
    case WIFI_MODE: wifiScan(); break;
    case BLE_MODE:  bleScan();  break;
    case VOLT_MODE:
      for(int i=0;i<SCREEN_WIDTH;++i) vTrace[i]=0.0f;
      drawAxesVolt(); renderLCDVolt(0.0f); break;
    case CURR_MODE:
      for(int i=0;i<SCREEN_WIDTH;++i) iTrace[i]=0.0f;
      drawAxesCurr(); renderLCDCurr(0.0f); break;
    case RF_MODE:
      for(int i=0;i<SCREEN_WIDTH;++i) rfTrace[i]=-100.0f;
      drawAxesRF(); renderLCDRF(0.0f, -100.0f); break;
    default: break;
  }
}
